...
# Derive the wallet address from the secret key (Hyperliquid needs both).
DERIVED_ADDR=""
if command -v python3.12 >/dev/null 2>&1; then
  DERIVED_ADDR="$(python3.12 -c "from eth_account import Account; print(Account.from_key('$HL_SECRET').address)" 2>/dev/null || true)"
fi
[ -z "$DERIVED_ADDR" ] && DERIVED_ADDR="$(python3 -c "from eth_account import Account; print(Account.from_key('$HL_SECRET').address)" 2>/dev/null || true)"
write_cfg () {
  local dir="$1"
